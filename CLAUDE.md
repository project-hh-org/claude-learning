...
| 2026-05-12 | ai-agent-debugging-silent-failure | AI 에이전트 디버깅: stderr 비어있는 실패와 SQL 검증 오탐 |
| 2026-05-17 | claude-memory-misapplication | Claude 메모리가 있어도 틀릴 수 있다 |
| 2026-05-15 | ai-agent-full-automation | AI 에이전트 완전 자동화 (git push까지 무인 실행) |
...
| claude-project-memory | Claude 프로젝트 메모리 패턴 (.claude/memory/) | evergreen |
| search-phantom-match-pattern | 검색 Phantom Match 패턴 (발골 사전, silent failure) | evergreen |
| hybrid-reflection-loop | Hybrid Reflection Loop (AI + 코드 검증 결합 루프) | evergreen |
| non-destructive-ai-agent | 비파괴적 AI 에이전트 설계 원칙 | evergreen |
| claude-cli-allowed-tools | Claude CLI --allowedTools server-side vs client-side 구분 | evergreen |
| clickhouse-json-extract | ClickHouse JSON 컬럼 필드 추출 패턴 | evergreen |
